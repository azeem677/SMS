    ...
    if (!project) {
        return (
            <div className="p-6 text-center text-zinc-900 dark:text-zinc-200">
                <p className="text-3xl md:text-5xl mt-40 mb-10">{t("Project not found")}</p>
    ...
    return (
        <div className="space-y-5 max-w-6xl mx-auto text-zinc-900 dark:text-white">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <span className={`px-2 py-1 rounded text-xs capitalize ${statusColors[project.status] || "bg-zinc-200"}`} >
                            {project.status ? t(project.status.replace("_", " ")) : t("Unknown")}
                        </span>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-2 sm:flex flex-wrap gap-6">
                {[
                    { label: t("Total Tasks"), value: tasks.length, color: "text-zinc-900 dark:text-white" },
                    { label: t("Completed"), value: tasks.filter((t) => t.status === "Done").length, color: "text-emerald-700 dark:text-emerald-400" },
                    { label: t("In Progress"), value: tasks.filter((t) => t.status === "In Progress" || t.status === "To Do").length, color: "text-amber-700 dark:text-amber-400" },
                    { label: t("Team Members"), value: project.members?.length || 0, color: "text-blue-700 dark:text-blue-400" },
                ].map((card, idx) => (
            ...
            <div>
                <div className="inline-flex flex-wrap max-sm:grid grid-cols-3 gap-2 border border-zinc-200 dark:border-zinc-800 rounded overflow-hidden">
                    {[
                        { key: "tasks", label: t("Tasks"), icon: FileStackIcon },
                        { key: "calendar", label: t("Calendar"), icon: CalendarIcon },
                        { key: "analytics", label: t("Analytics"), icon: BarChart3Icon },
